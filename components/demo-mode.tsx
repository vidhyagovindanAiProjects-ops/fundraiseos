...
export function DemoMode() {
  ...
  const approveExtraction = (extraction: Extraction, rawText: string) => {
    ...
    setUpload(false);
    setSelected(lp);
    notify("AI extraction approved and saved across LP Brain");
  };
...
function DashboardView({ profiles, tasks, feed, metrics, latestUploadId, fundDNA, strategy, bestFits, go, openLP, openChat, openUpload }: { profiles: LP[]; tasks: Task[]; feed: Feed[]; metrics: { total: number; active: number; warm: number; commitments: number; pipeline: number; open: number; overdue: number; score: number }; latestUploadId: string | null; fundDNA: FundDNA | null; strategy: FundraisingStrategy | null; bestFits: { lp: LP; fit: LPFit }[]; go: (s: Screen) => void; openLP: (lp: LP) => void; openChat: () => void; openUpload: () => void }) {
  ...
  return <>
    ...
    <section className="chief-grid">
      <div className="panel chief-card primary-chief">
        ...
        <p><b>Next move:</b> {strategy ? "Execute the recommended action, then ask Memory for the exact outreach or prep note." : fundDNA ? "Use the fit engine to prioritize warm LPs before broad outreach." : "Create Fund DNA so LP Brain can rank LP fit and generate strategy."}</p>
