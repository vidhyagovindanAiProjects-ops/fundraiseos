...
const integrationCatalog: Array<{ key: IntegrationKey; name: string; icon: typeof Mail; purpose: string; capabilities: string[]; downstream: string[]; authNote: string }> = [
  ...
  { key: "docsend", name: "DocSend", icon: FileText, purpose: "Track deck views and detect LP engagement.", capabilities: ["Track deck views", "Detect LP engagement", "Update LP interest signals"], downstream: ["Fundraising signals", "Pipeline stage", "Chief of Staff priorities"], authNote: "Demo engagement stream; production DocSend auth can attach later." },
...
export function DemoMode() {
  ...
  const discovery = useMemo(() => discoverInvestors(fundDNA, strategy, profiles, outcomeIntel), [fundDNA, strategy, profiles, outcomeIntel]);
  ...
  const syncIntegration = (key: IntegrationKey) => {
    ...
    setTimeout(() => {
      ...
      if (key === "meetings") setFeed((current) => [{ title: "Transcript sync queued Meeting Debrief Agent", meta: "Zoom / Google Meet demo transcript converted into meeting summary", tag: "Autonomous Engine" }, ...current]);
...
function discoverInvestors(dna: FundDNA | null, strategy: FundraisingStrategy | null, profiles: LP[], outcomeIntel: FundraisingOutcomeIntelligence): { opportunities: DiscoveryOpportunity[]; insights: DiscoveryInsights } {
...
function answerDiscoveryQuestion(low: string, discovery: ReturnType<typeof discoverInvestors>) {
  ...
  return `Top 10 new LPs to pursue this week:\n${top.map((opp) => `${opp.rank}. ${opp.lpName} - ${opp.organization} (${opp.investorType}). ${opp.confidenceScore}% confidence. Expected check: ${opp.expectedCheckSize}. Why: ${opp.whyMatches} Next: ${opp.suggestedNextAction}.`).join("\n")}`;
}
...
function DashboardView({ profiles, tasks, feed, metrics, latestUploadId, fundDNA, strategy, bestFits, go, openLP, openChat, openUpload, openOnboarding, workspaceMode, onboardingSummary, signals, forecast, autonomous, outcomeIntel, discovery, fitResults }: { profiles: LP[]; tasks: Task[]; feed: Feed[]; metrics: { total: number; active: number; warm: number; commitments: number; pipeline: number; open: number; overdue: number; score: number }; latestUploadId: string | null; fundDNA: FundDNA | null; strategy: FundraisingStrategy | null; bestFits: { lp: LP; fit: LPFit }[]; go: (s: Screen) => void; openLP: (lp: LP) => void; openChat: () => void; openUpload: () => void; openOnboarding: () => void; workspaceMode: WorkspaceMode; onboardingSummary: OnboardingSummary | null; signals: Record<string, FundraisingSignal>; forecast: ReturnType<typeof fundraisingForecast>; autonomous: AutonomousRecommendation[]; outcomeIntel: FundraisingOutcomeIntelligence; discovery: ReturnType<typeof discoverInvestors>; fitResults: Record<string, LPFit> }) {
  ...
  return <>
    ...
    <section className="panel discovery-chief">
      <div className="panel-head"><div><h2>Discover Investors</h2><p>Every morning LP Brain recommends three new LPs to pursue based on Fund DNA, outcomes, sector, stage, geography, and check-size fit.</p></div><button onClick={() => go("Discover Investors")}>Open discovery <ArrowRight /></button></div>
...
function Chat({ profiles, tasks, fundDNA, strategy, opportunities, outcomes, fitResults, outcomeIntel, integrations, discovery, close }: { profiles: LP[]; tasks: Task[]; fundDNA: FundDNA | null; strategy: FundraisingStrategy | null; opportunities: LPOpportunity[]; outcomes: Record<string, OpportunityOutcome>; fitResults: Record<string, LPFit>; outcomeIntel: FundraisingOutcomeIntelligence; integrations: IntegrationState; discovery: ReturnType<typeof discoverInvestors>; close: () => void }) { const [q, setQ] = useState(""); const [messages, setMessages] = useState<{ role: "user" | "ai"; text: string }[]>([]); const ask = (text: string) => { const answer = answerMemoryQuestion(text, profiles, tasks, fundDNA, strategy, opportunities, outcomes, fitResults, outcomeIntel, integrations, discovery); const autoNote = "\n\nAutonomous update: Ask Memory logged this question, refreshed priorities, checked LP signals, recalculated forecast context, and kept recommended actions synchronized with live memory."; setMessages((m) => [...m, { role: "user", text }, { role: "ai", text: answer + autoNote }]); setQ(""); }; const prompts = ["Who should I contact next?", "Which LP opportunities have the highest fit?", "Which LPs need a warm introduction?", "Draft outreach for the top LP opportunity.", "What are we learning from passed LPs?", "Which LP type is converting best?", "What should the GP do today?", "Why are LPs not converting?", "Which introduction source performs best?", "What should change in the strategy this week?", "Which integrations are connected?", "What did Gmail import?", "Discover the top new LPs this week", "Why does Sofia Almeida rank highly?"]; return <aside className="chat"><div className="chat-head"><span><Sparkles /></span><p><b>Fundraising chief of staff</b><small>• Grounded in {profiles.length} LP profiles{fundDNA ? " + Fund DNA" : ""}{strategy ? " + Strategy" : ""}{opportunities.length ? " + Opportunities" : ""}</small></p><button aria-label="Close chat" onClick={close}><X /></button></div><div className="chat-body">{messages.length ? <div className="messages">{messages.map((m, i) => <div className={m.role} key={i}><span style={{ whiteSpace: "pre-line" }}>{m.text}</span>{m.role === "ai" && <small><FileText />Source: live memory state</small>}</div>)}</div> : <><div className="chat-intro"><BrainCircuit /><h2>Ask what to do next.</h2><p>Answers use LP memory, Fund DNA, fit scores, fundraising strategy, LP opportunities, discovery signals, and integrations.</p></div><div className="suggestions">{prompts.map((x) => <button key={x} onClick={() => ask(x)}>{x}<ArrowRight /></button>)}</div></>}</div><form onSubmit={(e) => { e.preventDefault(); if (q.trim()) ask(q); }}><input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Ask about opportunities, strategy, intros, or outreach..." /><button aria-label="Send question"><Send /></button></form></aside>; }
