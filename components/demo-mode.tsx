...
type Screen = "Home" | "LP Pipeline" | "Meetings" | "Knowledge" | "Integrations" | "Settings" | "Fund DNA" | "Fundraising Strategy" | "LP Opportunities" | "LP Directory" | "Follow-ups" | "Relationship Graph";
...
export function DemoMode() {
  ...
  const nav: [Screen, typeof Home][] = [["Home", Home], ["LP Pipeline", Users], ["Meetings", LayoutList], ["Knowledge", BrainCircuit], ["Integrations", Webhook], ["Settings", Settings]];
...
function DashboardView({ profiles, tasks, feed, metrics, latestUploadId, fundDNA, strategy, bestFits, go, openLP, openChat, openUpload, openOnboarding, workspaceMode, onboardingSummary, signals, forecast, autonomous, outcomeIntel, fitResults }: { profiles: LP[]; tasks: Task[]; feed: Feed[]; metrics: { total: number; active: number; warm: number; commitments: number; pipeline: number; open: number; overdue: number; score: number }; latestUploadId: string | null; fundDNA: FundDNA | null; strategy: FundraisingStrategy | null; bestFits: { lp: LP; fit: LPFit }[]; go: (s: Screen) => void; openLP: (lp: LP) => void; openChat: () => void; openUpload: () => void; openOnboarding: () => void; workspaceMode: WorkspaceMode; onboardingSummary: OnboardingSummary | null; signals: Record<string, FundraisingSignal>; forecast: ReturnType<typeof fundraisingForecast>; autonomous: AutonomousRecommendation[]; outcomeIntel: FundraisingOutcomeIntelligence; fitResults: Record<string, LPFit> }) {
...
function KnowledgeWorkspace({ profiles, latestUploadId, fundDNA, strategy, bestFits, fitResults, opportunities, outcomes, outcomeIntel, saveDNA, setOutcome, openLP, openChat, go }: { profiles: LP[]; latestUploadId: string | null; fundDNA: FundDNA | null; strategy: FundraisingStrategy | null; bestFits: { lp: LP; fit: LPFit }[]; fitResults: Record<string, LPFit>; opportunities: LPOpportunity[]; outcomes: Record<string, OpportunityOutcome>; outcomeIntel: FundraisingOutcomeIntelligence; saveDNA: (dna: FundDNA) => void; setOutcome: (id: string, outcome: OpportunityOutcome) => void; openLP: (lp: LP) => void; openChat: () => void; go: (s: Screen) => void }) {
