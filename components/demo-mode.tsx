...
function Chat({ profiles, tasks, fundDNA, strategy, opportunities, outcomes, fitResults, outcomeIntel, integrations, discovery, providers, close }: { profiles: LP[]; tasks: Task[]; fundDNA: FundDNA | null; strategy: FundraisingStrategy | null; opportunities: LPOpportunity[]; outcomes: Record<string, OpportunityOutcome>; fitResults: Record<string, LPFit>; outcomeIntel: FundraisingOutcomeIntelligence; integrations: IntegrationState; discovery: ReturnType<typeof discoverInvestors>; providers: InvestorProvider[]; close: () => void }) { const [q, setQ] = useState(""); const [messages, setMessages] = useState<{ role: "user" | "ai"; text: string }[]>([]); const ask = (text: string) => { const answer = answerMemoryQuestion(text, profiles, tasks, fundDNA, strategy, opportunities, outcomes, fitResults, outcomeIntel, integrations, discovery, providers); const autoNote = "\n\nRelationship Intelligence update: LP Brain logged this question, refreshed LP priorities, checked relationship signals, recalculated fit context, and kept recommended actions synchronized with live memory."; setMessages((m) => [...m, { role: "user", text }, { role: "ai", text: answer + autoNote }]); setQ(""); }; const prompts = ["Who should I fundraise from?", "Which LP categories have the highest probability?", "Which LP categories should I avoid?", "Which LPs need a warm introduction?", "What is my most efficient fundraising path?", "Draft outreach for the top LP opportunity.", "What are we learning from passed LPs?", "Which LP type is converting best?", "What should the GP do this week?", "Why are LPs not converting?", "Which introduction source performs best?", "What should change in the strategy this month?", "Discover the top new LPs this week", "Why does Sofia Almeida rank highly?", "Which investor providers are connected?"]; return <aside className="chat"><div className="chat-head"><span><Sparkles /></span><p><b>LP matchmaker and strategist</b><small>• Grounded in {profiles.length} LP profiles{fundDNA ? " + Fund DNA" : ""}{strategy ? " + Strategy" : ""}{opportunities.length ? " + Opportunities" : ""}</small></p><button aria-label="Close chat" onClick={close}><X /></button></div><div className="chat-body">{messages.length ? <div className="messages">{messages.map((m, i) => <div className={m.role} key={i}><span style={{ whiteSpace: "pre-line" }}>{m.text}</span>{m.role === "ai" && <small><FileText />Source: live memory state</small>}</div>)}</div> : <><div className="chat-intro"><BrainCircuit /><h2>Ask who to target, avoid, access, and convert.</h2><p>Answers use Fund DNA, LP personas, fit scores, fundraising strategy, LP opportunities, discovery signals, relationship intelligence, and learning data.</p></div><div className="suggestions">{prompts.map((x) => <button key={x} onClick={() => ask(x)}>{x}<ArrowRight /></button>)}</div></>}</div><form onSubmit={(e) => { e.preventDefault(); if (q.trim()) ask(q); }}><input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Ask who to target, avoid, access, or convert..." /><button aria-label="Send question"><Send /></button></form></aside>; }
