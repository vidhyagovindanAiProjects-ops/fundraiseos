...
function Upload({ close, approve }: { close: () => void; approve: (extraction: Extraction, rawText: string) => void }) {
  ...
  return <div className="backdrop"><div className="upload"><div className="modal-head"><div><span>{extraction ? "REVIEW EXTRACTION" : "AI MEETING EXTRACTION"}</span><h2>{extraction ? "Review before saving" : "Upload or paste meeting note"}</h2><small>{extraction ? "Approve or edit the structured JSON fields." : "OpenAI extracts LP data when configured. Demo fallback stays local."}</small></div><button aria-label="Close upload" onClick={close}><X /></button></div>{!extraction ? <><div className="drop" onClick={() => input.current?.click()}><input ref={input} hidden type="file" accept=".txt,.pdf,.docx" onChange={(e) => setFile(e.target.files?.[0] || null)} /><UploadCloud /><b>{file ? file.name : "Upload .txt meeting note"}</b><p>Paste text below for PDF/DOCX transcripts in Phase 1</p></div><div style={{ padding: "0 22px 18px" }}><textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder="Paste meeting note or transcript here..." style={{ width: "100%", minHeight: 145, border: "1px solid #e0e2e3", borderRadius: 10, padding: 12, resize: "vertical", fontSize: 12 }} /><button className="sample-upload" onClick={() => extract(true)}><Sparkles />Use sample Nora Ellis meeting note</button>{error && <p style={{ color: "#b45a42", fontSize: 11, lineHeight: 1.5 }}>{error}</p>}</div></> : <ReviewExtraction extraction={extraction} setField={setField} />}<div className="modal-actions"><button onClick={close}>Cancel</button>{!extraction ? <button className="primary" disabled={busy || (!note.trim() && !file)} onClick={() => extract(false)}><Sparkles />{busy ? "Extracting..." : "Extract with AI"}</button> : <button className="primary" onClick={() => approve(extraction, rawText || note || sampleMeetingNote)}>Approve and update FundraiseOS <ArrowRight /></button>}</div></div></div>;
}
