...
export async function POST(request: Request) {
  ...
  try {
      const result = await generateStructuredJSON({
        ...
        system: "Extract meeting intelligence for an emerging VC fundraising LP conversation. Return only JSON with keys: lpName, organization, lpType, conciseMeetingSummary, questionsAsked, objections, positiveSignals, negativeSignals, commitmentsOrPromises, followUpTasks, nextAction, nextActionDate, suggestedPipelineStage, assumptions, evidence. evidence must cite source snippets from the note for every material conclusion. Do not invent LP investment history, check size, preferences, or personal details. Do not change pipeline stage; only suggest one.",
        user: read.text,
      });
      if (!result.parsed) throw result.error || new Error("OpenAI returned invalid meeting JSON. Please retry.");
      const extraction = normalizeMeetingExtraction(result.parsed);
      ...
      const legacyExtraction = {
        lpName: clean(parsed.lpName),
        firm: clean(parsed.organization),
        investorType: clean(parsed.lpType) || "Unknown",
        meetingDate: "",
